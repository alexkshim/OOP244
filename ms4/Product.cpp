...
namespace sict {
	...
	void Product::sku(const char* sku) { strncpy(sku_, sku, MAX_SKU_LEN); }
	...
	double Product::cost()const {
		if (taxed()) {
			return this->price_ + (this->price_ * TAX);
		}
		else {
			return this->price_;
		}
	...
	bool Product::isEmpty()const { return sku_[0] == '\0' && name_ == nullptr && price_ == 0 && quantity_ == 0 && qtyNeeded_ == 0 && taxed_ == false; }
	...
	int Product::operator+=(int qtyUp) {
		quantity_ += qtyUp;
		return quantity_;
	}
	...
	int Product::operator-=(int qtyDown) {
		quantity_ -= qtyDown;
		return quantity_;
	}
