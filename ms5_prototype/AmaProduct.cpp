	...
	std::ostream& AmaProduct::write(ostream& ostr, bool linear)const {
		if (!this->err_.isClear()) {
			...
			if (linear == true) {
				...
				ostr.setf(ios::fixed);
				ostr.setf(ios::left);
				ostr.width(MAX_UPC_LEN); // may use setw instead of this here
				ostr << sku() << '|';
				...
				ostr.setf(ios::right);
				ostr.width(7);
				ostr << fixed << setprecision(2) << fabs(cost()) << '|';
				...
				ostr << "Quantity On Hand: " << quantity() << " kg" << endl;
				ostr << "Qunatity Needed: " << qtyNeeded() << endl;
			}
