...
namespace sict {
	...
	std::ostream& AmaProduct::write(ostream& ostr, bool linear)const {
		if (!this->err_.isClear()) {
			...
			if (linear == true) {
				...
				ostr << "Sku: " << sku() << endl;
				ostr << "Name: " << name() << endl;
				ostr << "Price: " << fixed << setprecision(2) << fabs(price()) << endl;
	...
	std::istream& AmaProduct::read(istream& istr) {
		...
		if (!istr.fail()) {
			...
			cin.ignore();
			cout << "Sku: ";
			istr.getline(tempSku, MAX_GETLINE_NUM, '\n');
			sku(tempSku);

			if (!istr.fail()) {
				cout << "Name: ";
				istr.getline(tempName, MAX_GETLINE_NUM, '\n');
				name(tempName);

				if (!istr.fail()) {
					cout << "Unit: ";
					istr.getline(tempUnit, MAX_GETLINE_NUM, '\n');
					unit(tempUnit);
