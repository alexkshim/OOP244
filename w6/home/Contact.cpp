...
namespace sict {
	...
	Contact::Contact(const Contact& obj) {
		...
		strncpy(contactName, obj.contactName, MAX_NAME_LENGTH);
		contactName[MAX_NAME_LENGTH - 1] = '\0';
		phoneNum = new long long[obj.numOfPhoneNum];
		for (int i = 0; i < obj.numOfPhoneNum; i++) {
			if (obj.phoneNum[i] < 1000000000000 && obj.phoneNum[i] >= 10000000000) {
				phoneNum[numCount] = obj.phoneNum[i];
				numCount++;
			}
		}
	...
	Contact& Contact::operator+=(long long obj) {

		Contact temp;

		int i;
		if (obj < 1000000000000 && obj >= 10000000000) {
			if (this->numOfPhoneNum == 0) {
				this->numOfPhoneNum += 1;
				this->phoneNum = new long long[this->numOfPhoneNum];
				this->phoneNum[0] = obj;
			}
			else {
				temp.phoneNum = new long long[this->numOfPhoneNum];
				for (i = 0; i < this->numOfPhoneNum; i++) {
					temp.phoneNum[i] = this->phoneNum[i];
				}
				this->numOfPhoneNum += 1;

				delete[] this->phoneNum;
				this->phoneNum = new long long[this->numOfPhoneNum];
				for (i = 0; i < this->numOfPhoneNum - 1; i++) {
					this->phoneNum[i] = temp.phoneNum[i];
				}
				this->phoneNum[i] = obj;
			}
